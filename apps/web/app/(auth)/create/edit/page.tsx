...
import type { GifResolution, GifFps } from '@gifstudio/shared';
import { GIF_CONSTRAINTS } from '@gifstudio/shared';
...
export default function EditPage() {
  ...
  const { draft, setTrim, setGifResult, clear } = useDraft();
  ...
  const setInPoint = useCallback(() => {
    ...
    // On pose le nouveau début = now
    let newStart = Math.max(0, Math.min(now, videoDuration));
    let newEnd = currentEnd;

    // Si ça casse la durée min ou inverse, on adapte la fin
    if (newEnd - newStart < minDur) {
      newEnd = Math.min(videoDuration, newStart + minDur);
    }

    // Si la durée dépasse le max, on contraint la fin
    if (newEnd - newStart > maxDur) {
      newEnd = newStart + maxDur;
    }

    // Si on ne peut pas poser (ex: la vidéo est trop courte à partir de newStart)
